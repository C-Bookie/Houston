...
from scrambler.audio_streams import open_stream, get_device_info, get_api_by_name
...
sample_rate = 48000  # int(input_device_info["defaultSampleRate"])
channels = 2  # min(input_device_info["maxInputChannels"], output_device_info["maxOutputChannels"])
stream_format = p.get_format_from_width(channels)  # pyaudio.paInt16

stream = p.open(
    format=stream_format,
    channels=channels,
    rate=sample_rate,
    frames_per_buffer=sample_rate // 60,
    # input=True,
...
def openInputStream(device_id=None, framerate=None):
    return stream, input_device_info


def openOutpoutStream(device_id=None, framerate=None):
    return stream, output_device_info
