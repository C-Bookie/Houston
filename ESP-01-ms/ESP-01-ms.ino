...
void connectionSanity() {
  while (!client.connected()) {
    ...
    if (client.connect(host, port)) {
      printStatus();
      send_msg("{\"type\": \"register\", \"args\": [\"motion_sensor\", 2077]}");
...
int sensor_pin = 0;
int sensor_state_last = false;
int sensor_state_now = false;
char* state_msg;
...
void loop() {
  ...
  sensor_state_now = !sensor_state_now;
//  sensor_state_now = digitalRead(sensor_pin);
  if (sensor_state_now != sensor_state_last) {
    char* msg = "{\"type\": \"broadcast\", \"args\": [{\"type\": \"sensor\", \"args\": [";
    if (sensor_state_now)
      msg = strcat(msg, "true");
    else
      msg = strcat(msg, "false");
     msg = strcat(msg, "]}]}");
    send_msg(msg);
    sensor_state_last = sensor_state_now;
  }
